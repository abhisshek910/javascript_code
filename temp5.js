(function() {
    ...
    function jsonldVis(keystoremove, high, jsonld, selector, config) {
        ...
        svg.call(tip);
        if (keystoremove) {
            jsonld = removeKeys(jsonld, keystoremove)
        }
            ...
            if (obj && typeof obj === "object") {
                return Object.keys(obj)
                    ...
                    .reduce(
                        (acc, key) => ({
                            ...acc,
                            [key]: removeKeys(obj[key], keysToRemove),
                        }), {}
                    );
            }
            return obj;
        ...
        function jsonldTree(source) {
            ...
            Object.keys(source).forEach(function(key) {
                ...
                if (high.includes(key) && typeof key === 'string' && source[key].length <= 1) {
              ...
              else if(high.includes(key) && typeof source[key] === 'string')
              {
                valueExtended = source[key];
                    value = valueExtended;
                    if (value.length > maxLabelWidth / 9) {
